...
function sequence(includeMediaHash = true): AnnotatedBenchmarkSequence {
  return {
    schemaVersion: '1',
    datasetId: 'one-frame-mobility',
    sequenceId: 'seq-001',
    source: {
      annotationSha256: HASH_A,
      ...(includeMediaHash ? { mediaSha256: HASH_B } : {}),
    },
    frames: [{
      frameId: 'f1',
      timestampMs: 10_000,
      ...
      objects: [{
        ...
        className: 'person',
        bbox: { x: 200, y: 50, width: 400, height: 200 },
        occlusion: 'none',
      }],
    }],
  };
...
describe('external candidate benchmark session', () => {
  it('produces a valid selection-profile report when model, corpus and temporal evidence are complete', async () => {
    ...
    expect(result.report.benchmark.frameCount).toBe(1);
    expect(result.report.benchmark.classMetrics).toHaveLength(1);
    ...
    expect(result.report.corpus.annotationSha256).toBe(HASH_A);
    expect(result.report.corpus.mediaSha256).toBe(HASH_B);
    expect(result.validity.status).toBe('valid');
    expect(progress).toEqual(['1/1:f1']);
    expect(factory.session.releaseCount).toBe(1);
  });

  it('keeps detector metrics but marks selection evidence invalid when the timed medium hash is missing', async () => {
    const result = await runExternalCandidateBenchmarkSession(
      OPENCV_SSD_MOBILENET_V2_COCO_2026JUL,
      artifact(),
      diagnostic(),
  ...
  it('can keep the same incomplete evidence provisional under the development profile', async () => {
    const result = await runExternalCandidateBenchmarkSession(
      OPENCV_SSD_MOBILENET_V2_COCO_2026JUL,
      artifact(),
      diagnostic(),
