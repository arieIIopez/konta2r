...
import {
  ...
  type OnnxProbeRecord,
} from '../detection/onnx/probeRecord';
...
export class ModelProbePanel {
  ...
  private lastRecord: OnnxProbeRecord | null = null;
      ...
      <section class="node-runtime-shell probe-shell">
        ...
        <div class="probe-card">
          ...
          <div class="node-runtime-controls">
            ...
            ${record ? '<button class="action secondary" type="button" data-probe-copy>Copiar registro JSON</button><button class="action secondary" type="button" data-probe-download>Guardar registro JSON</button>' : ''}
          </div>
          <div class="runtime-error hidden" data-probe-error></div>
          <div class="probe-progress ${record ? '' : 'hidden'}" data-probe-progress>${record ? `Último probe: ${record.probedAtIso} · metadata ${record.metadataCompleteness}` : ''}</div>
          <pre class="probe-output ${record ? '' : 'hidden'}" data-probe-output>${record ? this.escapeHtml(serializeOnnxProbeRecord(record)) : ''}</pre>
        </div>
      </section>
    `;
  ...
  private async runProbe(): Promise<void> {
    ...
    try {
      ...
      this.lastRecord = buildOnnxProbeRecord(this.candidate, artifact, result);
      this.running = false;
      this.render();
    } catch (error) {
      if (this.destroyed) return;
  ...
  private async copyRecord(): Promise<void> {
    const record = this.lastRecord;
    if (!record) return;
    const text = serializeOnnxProbeRecord(record);
    if (!navigator.clipboard?.writeText) {
      this.showTransientError('Clipboard API no disponible. Usa “Guardar registro JSON”.');
      return;
    }
    try {
      await navigator.clipboard.writeText(text);
      const progress = this.mountElement?.querySelector<HTMLElement>('[data-probe-progress]');
      if (progress) progress.textContent = `Registro copiado · ${record.probedAtIso}`;
    } catch (error) {
  ...
  private downloadRecord(): void {
    const record = this.lastRecord;
    if (!record) return;
    const blob = new Blob([serializeOnnxProbeRecord(record)], { type: 'application/json;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `konta2r-probe-${record.candidateId}-${safeFileTimestamp(record.probedAtIso)}.json`;
