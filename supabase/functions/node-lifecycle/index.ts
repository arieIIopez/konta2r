...
function lifecycleBody(value: unknown): { nodeId: string; action: NodeLifecycleAction } | undefined {
  ...
  return {
    nodeId: body.nodeId,
    action: body.action as NodeLifecycleAction,
  };
}
...
Deno.serve(async (request: Request) => {
  ...
  try {
    // Lifecycle mutations are privileged server writes. Even with verify_jwt=true,
    // re-resolve the authenticated user before using owner_user_id in SQL.
    ...
    if (body.action === 'rotate') {
      ...
      pepper = pepperForKeyVersion(keyVersion);
    ...
    if (message === 'Node not found for authenticated owner') {
      return jsonResponse({ code: 'node_not_found' }, 404);
    }
    if (
    ...
    if (message === 'Invalid Konta2r node id') {
      return jsonResponse({ code: 'invalid_lifecycle_payload' }, 422);
    }
